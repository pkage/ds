...
def parse_hooks(config: Config, key: str = "workspace.metadata.git-hooks") -> Hooks:
    """Git hooks are not officially defined `Cargo.toml`.

    However, we'll look at:
    - `workspace.metadata.git-hooks`
    - `package.metadata.git-hooks`
    """

    if get_key(config.data, key, KEY_MISSING) is KEY_MISSING:
        key = "package.metadata.git-hooks"
        if get_key(config.data, key, KEY_MISSING) is KEY_MISSING:
            # return nothing
            return {}
    return ds_toml.parse_hooks(config, key)
